...
var _Sources = (() => {
  ...
  var KaganeInfo = {
    version: "1.0.6",
    // J'augmente la version pour forcer la mise à jour
    name: "Kagane",
    icon: "icon.png",
    author: "Toi",
  ...
  var Kagane = class extends import_types2.Source {
    ...
    parseMangaListFromHTML(html3) {
      const $2 = load(html3);
      const items = [];
      $2('a[href^="/series/"]').each((i, el) => {
        const href = $2(el).attr("href");
        const id = href?.split("/").pop();
        const title = $2(el).find("h3, h4, span.font-bold, .title").first().text().trim() || $2(el).attr("title");
    ...
    async getMangaDetails(mangaId) {
      ...
      const $2 = load(html3);
      const title = $2("h1").first().text().trim() || "Titre Inconnu";
      const desc = $2('p.description, .summary, div[class*="description"]').text().trim();
      let image = $2('img[alt*="cover"], img[alt="' + title + '"]').attr("src") || "";
      if (image.startsWith("/")) image = DOMAIN + image;
      let status = "Ongoing";
      if (html3.includes('"status":"COMPLETED"') || $2('*:contains("Status: Completed")').length > 0) {
        status = "Completed";
      }
    ...
    async getChapters(mangaId) {
      const request = App.createRequest({
        ...
        // L'API v1 renvoie souvent tout ici
        method: "GET",
        headers: COMMON_HEADERS
      });
      const response = await this.requestManager.schedule(request, 1);
